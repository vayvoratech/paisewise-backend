...
CREATE TABLE portfolio.holdings (
                                    ...
                                    symbol              VARCHAR(30) NOT NULL REFERENCES practice.symbols(symbol),  -- Issue 8 fix, Option B
...
CREATE INDEX idx_ledger_user_id ON portfolio.ledger(user_id, id DESC);
CREATE INDEX idx_ledger_ref ON portfolio.ledger(ref_type, ref_id) WHERE ref_id IS NOT NULL;

-- Same append-only enforcement pattern as auth.audit_log (Issue 9 fix —
-- application_user role is created once in auth-service's V2 migration,
-- which must run before this file since roles are cluster-wide).
-- Fix (found during testing): USAGE on the schema itself must also be
-- granted, otherwise application_user can't even SELECT/INSERT — the
-- original design only revoked/granted table-level privileges and
-- missed this, which would have silently broken normal app writes.
...
-- Fix (found during testing): id is BIGSERIAL, which is backed by a
-- sequence — INSERT fails without USAGE on that sequence, even with
-- INSERT granted on the table itself.
...
CREATE INDEX idx_mf_schemes_name ON portfolio.mf_schemes USING gin(to_tsvector('english', scheme_name || ' ' || amc_name));
CREATE INDEX idx_mf_schemes_category ON portfolio.mf_schemes(category, risk_level) WHERE is_active = true;
CREATE INDEX idx_mf_schemes_returns ON portfolio.mf_schemes(returns_1y DESC, returns_3y DESC) WHERE is_active = true;
CREATE INDEX idx_mf_schemes_elss ON portfolio.mf_schemes(is_tax_saver) WHERE is_tax_saver = true AND is_active = true;
...
CREATE INDEX idx_mf_investments_user_id ON portfolio.mf_investments(user_id, created_at DESC);
CREATE INDEX idx_mf_investments_scheme ON portfolio.mf_investments(scheme_code);
...
CREATE INDEX idx_sips_user_id ON portfolio.sips(user_id, status);
CREATE INDEX idx_sips_next_debit ON portfolio.sips(next_debit_date, status) WHERE status = 'ACTIVE';
CREATE INDEX idx_sips_failed ON portfolio.sips(installments_failed) WHERE status = 'ACTIVE' AND installments_failed > 0;

-- =====================================================================
-- TABLE 16: portfolio_insights
-- =====================================================================
CREATE TABLE portfolio.portfolio_insights (
                                              id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                                              user_id                 UUID NOT NULL REFERENCES auth.users(id),
...
CREATE INDEX idx_portfolio_insights_user_date ON portfolio.portfolio_insights(user_id, insight_date DESC, language);
CREATE INDEX idx_portfolio_insights_fallback ON portfolio.portfolio_insights(insight_date, generation_status)
